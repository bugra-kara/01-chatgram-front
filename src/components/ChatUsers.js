...
const ChatUsers = ({socket, sendMessage, onlineUsers, modalOpen}) => {
    ...
    useEffect(()=> {
        console.log(isSelect, newMsg, userAdded );
        if(!isSelect || newMsg || userAdded) {
            const handleChats = async ()=> {
                try {
                    if(state.userId !== null) {
                        const resp = await axios.get(`https://chatgram.onrender.com/api/v1/chats/${state.userId}`,{withCredentials: true})
                        getAllChatsContext(resp.data.data)
                    }
                    else {
                        console.log(state);
                        const resp = await axios.get(`https://chatgram.onrender.com/api/v1/chats/${state.user.id}`,{withCredentials: true})
                        getAllChatsContext(resp.data.data)
                    }
                } catch (error) {
                    console.log(error);
