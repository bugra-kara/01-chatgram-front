...
import { Link, Outlet, redirect, Navigate, useNavigate } from 'react-router-dom'
import { ToastContainer } from 'react-toastify';
import { useAuthContext } from '../context/authContext'
function Login() {
    const { handleSubmitLogin, handleChange, state } = useAuthContext()
    const navigate = useNavigate()
    useEffect(()=> {
        if(state.userId !== null || state.user !== null) {
            navigate('/')
        }
